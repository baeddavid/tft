...
const api = TeemoJS(process.env.RIOT_KEY);
...
export async function getSummonerNameFromPuuid(puuid) {
    const summoner = await api.get("na1", "summoner.getByPUUID", puuid);
    return summoner.name;
}

export async function getSummonerTftMatchHistory(summonerName) {
